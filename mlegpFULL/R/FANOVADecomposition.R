`FANOVADecomposition` <-
function(gp, Interaction = TRUE, verbose=TRUE, outputs = NULL, lower = NULL, upper = NULL, maxpts = NULL, parallel = FALSE) {
	if (gp$numDim == 1) {
	...
	#NextMethod("FANOVADecomposition", gp, Interaction = Interaction, verbose=verbose, outputs = outputs, lower = lower, upper = upper, parallel = parallel)	
  UseMethod("FANOVADecomposition", gp)
}
